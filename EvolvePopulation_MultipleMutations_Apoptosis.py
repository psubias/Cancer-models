...
class EA():
		...
		if (self.spatial == True):
			for i in range(self.N):
				...
				if (np.random.uniform() < population[0][cell2rep[0]][cell2rep[1]]):
					...
					cell2rep_Nam, cell2rep_Nim = self.mutate_individual(population[1][cell2rep[0]][cell2rep[1]], \
					  population[2][cell2rep[0]][cell2rep[1]])
					cell2rep_r = self.compute_r(cell2rep_Nam, cell2rep_Nim)
					
					apoptosis = self.compute_apoptosis(cell2rep_Nim)
					
					if (np.random.uniform() < apoptosis):
						# Apoptosis implies new cell occupies replication cell space
						population[0][cell2rep[0]][cell2rep[1]] = self.compute_r(0, 0)
						population[1][cell2rep[0]][cell2rep[1]] = 0
						population[2][cell2rep[0]][cell2rep[1]] = 0
					  
					else:
						population[0][cell2sub_row][cell2sub_col] = cell2rep_r
						population[1][cell2sub_row][cell2sub_col] = cell2rep_Nam
						population[2][cell2sub_row][cell2sub_col] = cell2rep_Nim
		else:
			for i in range(self.N):
				...
				if (np.random.uniform() < population[0][cell2rep]):
					...
					cell2rep_Nam, cell2rep_Nim = self.mutate_individual(population[1][cell2rep], \
					  population[2][cell2rep])
					cell2rep_r = self.compute_r(cell2rep_Nam, cell2rep_Nim)
					...
					if (np.random.uniform() < apoptosis):
						# Apoptosis implies new cell occupies replication cell space
						population[0][cell2rep] = self.compute_r(0, 0)
						population[1][cell2rep] = 0
						population[2][cell2rep] = 0
					else:
						population[0][cell2sub] = cell2rep_r
						population[1][cell2sub] = cell2rep_Nam
						population[2][cell2sub] = cell2rep_Nim
	...
	def compute_apoptosis(self, Nim):
		""" Computes apoptosis as a factor of the mutant TSGs a cell has. """
		apoptosis = 1 - (Nim * 1. / self.Ni)
		return(apoptosis)
...
if __name__ == "__main__":
	...
	output_dir = os.path.dirname(os.path.abspath(__file__)) + "/Results EvolvePopulation.py/apoptosis/"
