...
class EA():
	...
	def store_results(self, population):
		...
		self.evo_fitness1.append(copy.copy(self.compute_fitness_approach11()))	
		self.evo_fitness2.append(copy.copy(self.compute_fitness_approach21()))
...
if __name__ == "__main__":
	...
	size_individuals = [1e1, 1e2, 1e3, 1e4, 1e5]
	mu_list = [0.001, 0.03]
	for mu in mu_list:
		for N in size_individuals:
			...
			for spatial in [False, True]:
				for i in range(1, Na):
					...
					#df.to_csv(output_dir + "fitness1.csv", index = False, header = False, sep = ";", decimal = ".")
					np.savetxt(output_dir + "tseries_fitness11.txt", df, fmt='%.4f')
					
					df = pd.DataFrame(fitness2_global)
					#df.to_csv(output_dir + "fitness2.csv", index = False, header = False, sep = ";", decimal = ".")
					np.savetxt(output_dir + "tseries_fitness21.txt", df, fmt='%.4f')
